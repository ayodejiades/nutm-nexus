...
interface CourseMetadata {
  ...
  departments?: string[]; level?: number; credits?: number; semester?: "I" | "II";
}
...
async function fetchCourseDetails(slug: string): Promise<CourseDetails | null> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // FIX: Removed unused var from catch block
      try {
          const jsonError = JSON.parse(errorBody);
          details = jsonError.error || jsonError.message || details;
       } catch { /* Ignore parsing error */ } // <-- FIXED HERE
...
export default function CourseDetailPage() {
  ...
  // --- Styles (now assume dark mode base) ---
  const sectionStyle = "mb-8 p-6 border border-gray-700/50 rounded-lg bg-gray-800/30 shadow-sm";
  const sectionTitleStyle = "text-xl sm:text-2xl font-semibold mb-5 text-gray-100 border-b border-gray-700 pb-3";
  const listItemStyle = "flex items-center group py-2.5";
  const linkStyle = "font-medium text-primary-light hover:text-primary flex items-center break-words transition-colors duration-150 min-w-0";
  const moodleIconStyle = "w-5 h-5 mr-3 text-primary-light/70 flex-shrink-0";
  const fileIconStyle = "w-5 h-5 mr-3 text-gray-400 flex-shrink-0";
  const externalIconStyle = "w-4 h-4 ml-1.5 text-gray-500 inline-block flex-shrink-0 group-hover:text-primary";
  const metadataBadgeStyle = "text-xs text-gray-400 border border-gray-600 px-1.5 py-0.5 rounded whitespace-nowrap";

  return (
    <div className="max-w-4xl mx-auto">
      {/* Header */}
      <Link href="/" className="text-primary-light hover:underline mb-6 inline-block transition duration-150 text-sm"> ← Back to All Courses </Link>
      <div className="mb-5 border-b border-gray-700 pb-5">
        ...
        <h1 className="text-3xl md:text-4xl font-bold text-gray-100 mb-2">{metadata.title}</h1>
        {metadata.instructor && <p className="text-sm text-gray-400">Instructor: {metadata.instructor}</p>}
         {metadata.departments && metadata.departments.length > 0 && (
            <div className="text-sm text-gray-400 mt-2">
                <span className="font-medium text-gray-300">Departments:</span> {metadata.departments.join(', ')}
            </div>
         )}
      </div>
      <p className="text-lg text-foreground/70 mb-8">{metadata.description}</p>

      {/* Resources Section */}
      <div className={sectionStyle}>
         <h2 className={sectionTitleStyle}>Resources & Files</h2>
         {files && files.length > 0 ? (
           <ul className="space-y-1 divide-y divide-gray-800/50">
             {files.map((file) => {
                const fileName = file?.name || 'Unnamed File';
                const IconComponent = getFileIcon(fileName);
                return ( <li key={`${fileName}-${file.url || ''}`} className={`${listItemStyle} justify-between gap-4`}> <a href={file.url || '#'} target="_blank" rel="noopener noreferrer" className={linkStyle} download={!file.url?.startsWith('http')} aria-disabled={!file.url} title={file.url ? fileName : `${fileName} (Link unavailable)`}> <IconComponent className={fileIconStyle} aria-hidden="true"/> <span className="truncate" title={fileName}>{fileName}</span> </a> <span className="text-gray-400 text-xs ml-2 flex-shrink-0 whitespace-nowrap">{formatBytes(file.size)}</span> </li> );
             })}
           </ul>
         ) : ( <EmptyState title="No Resources Found" message="No files have been uploaded." /> )}
      </div>
      ...
      {metadata.youtubePlaylistId && (
         <div className={sectionStyle}>
            ...
            <div className="aspect-w-16 aspect-h-9 rounded-lg overflow-hidden shadow-md border border-gray-800 bg-gray-900">
              <iframe key={slug + '-' + metadata.youtubePlaylistId} src={`https://www.youtube.com/embed/videoseries?list=${metadata.youtubePlaylistId}`} title="YouTube video playlist player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen className="w-full h-full" loading="lazy"></iframe>
            </div>
         </div>
      )}
    </div>
