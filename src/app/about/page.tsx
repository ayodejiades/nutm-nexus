    ...
    <div className="max-w-5xl mx-auto space-y-20 animate-fade-in">
      ...
      <section className="text-center pt-8 sm:pt-12 relative">
        ...
        <div className="relative z-10">
          <p className="text-[10px] font-semibold text-primary uppercase tracking-[0.3em] mb-4">About the Platform</p>
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-semibold text-white tracking-tight mb-6">
            What is Nexus?
          </h1>
          <p className="text-base sm:text-lg text-foreground/70 max-w-2xl mx-auto font-medium leading-relaxed">
      ...
      <section className="relative">
        <div className="bg-surface-1/40 border border-white/5 rounded-2xl p-8 sm:p-10 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-semibold text-primary uppercase tracking-[0.3em] mb-3">Our Mission</p>
            <h2 className="text-2xl sm:text-3xl font-semibold text-white tracking-tight mb-6 max-w-lg">
      ...
      <section>
        <div className="text-center mb-12">
          <p className="text-[10px] font-semibold text-primary uppercase tracking-[0.3em] mb-3">Features</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {features.map((feature) => (
            ...
            >
              ...
              <p className="text-[13px] text-foreground/70 leading-relaxed">{feature.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section>
        <div className="text-center mb-12">
          <p className="text-[10px] font-semibold text-primary uppercase tracking-[0.3em] mb-3">Under the Hood</p>
          <h2 className="text-2xl sm:text-3xl font-semibold text-white tracking-tight">
            Technology Stack
          </h2>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {techStack.map((tech) => (
            ...
            >
              <p className={`text-sm font-semibold tracking-tight ${tech.color}`}>{tech.name}</p>
              <p className="text-[10px] font-bold text-foreground/60 uppercase tracking-widest mt-1">{tech.role}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-surface-1/30 border border-white/5 rounded-2xl p-6 sm:p-8">
        <p className="text-[10px] font-semibold text-primary uppercase tracking-[0.3em] mb-2">How It Works</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          {[
            ...
            <div key={item.step} className="flex flex-col">
              <span className="text-4xl font-semibold text-primary/20 mb-2 leading-none">{item.step}</span>
              <h3 className="text-sm font-semibold text-white mb-1.5 uppercase tracking-wider">{item.title}</h3>
              <p className="text-[13px] text-foreground/70 leading-relaxed">{item.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="text-center pb-8 border-t border-white/5 pt-12">
        ...
        <p className="text-sm text-foreground/70 max-w-md mx-auto mb-8">
          Head back to the course catalog and start browsing your materials.
        </p>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link
            href="/"
            className="px-8 py-4 bg-primary hover:bg-primary-light text-black rounded-xl font-semibold text-xs uppercase tracking-widest transition-all active:scale-95"
          >
            Browse Courses
          </Link>
          <Link
            href="/team"
            className="px-8 py-4 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-xl font-semibold text-xs uppercase tracking-widest transition-all active:scale-95"
          >
            Meet the Team
          </Link>
        </div>
