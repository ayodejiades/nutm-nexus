...
export async function GET() {
  ...
  if (!octokit || !owner || !repo) {
    const missing = [!octokit && "Octokit (Check GITHUB_TOKEN)", !owner && "GITHUB_REPO_OWNER", !repo && "GITHUB_REPO_NAME"].filter(Boolean).join(', ');
    console.error(`API Route Error: Missing configuration (${missing}).`);
